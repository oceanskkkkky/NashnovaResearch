...
量价正交化必须基于完整板块横截面，不得只对最终1—3个入选板块回归。优先使用同一分类体系（推荐申万一级/二级之一，单次运行不得混用）建立不少于20个板块的计算池，采集各板块1/5/20日收益与同口径主力资金净流入强度。资金强度优先按“净流入额/板块流通市值”标准化；流通市值缺失时可用“净流入额/板块成交额”，但同次运行必须统一。计算池覆盖率不足80%时按 `scoring.md` 执行降级，不得用少数候选板块拟合回归。

拥挤度数据源（评分规则见 `scoring.md`）：板块指数至少250个交易日日K，用于计算 RSI(14) 及其近一年历史分位；板块换手率与成交额占全市场比重均先取近5日均值，再与各自近一年滚动分布比较。另保存5个交易日前的综合拥挤度，用于 `ΔC5` 加速度预警。板块指数历史数据不可用时，可用板块成交额前5的头部成份股按成交额加权近似，但必须在台账注明近似口径；缺失1项时用其余两项计算并降置信度，缺失2项及以上时按 `C=50` 降级且不得声称低拥挤。
