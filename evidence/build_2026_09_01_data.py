...
NODE = "C:/Users/tizytian/.workbuddy/binaries/node/versions/22.22.2-2/node.exe"
WESTOCK = "C:/Users/tizytian/.workbuddy/binaries/node/workspace/node_modules/westock-data-skillhub/index.js"
PACKAGE = "westock-data-skillhub@1.0.5 (local install)"
HITHINK = "C:/Users/tizytian/.workbuddy/binaries/node/workspace/node_modules/@hithink-tech/hithink-finance-cli/dist/cli/main.js"
...
def westock(*args: str, retries: int = 2):
    return execute([NODE, WESTOCK, *args], retries=retries)


def hithink(*args: str, retries: int = 2):
    return execute([NODE, HITHINK, *args], retries=retries)
...
def run_main():
    ...
    def max_macro_date(exclude_prefixes=("FORECAST", "MLF")):
        values = []
        for row in macro_effective:
    ...
    def max_macro_date_len(length: int):
        values = []
        for row in macro_effective:
            for key, value in row.items():
                if key.startswith("FORECAST"):
                    continue
                if isinstance(value, int) and len(str(value)) == length:
                    values.append(value)
        return max(values) if values else None

    macro_latest_date = max_macro_date()
    macro_latest_monthly = max_macro_date_len(6)
    macro_latest_quarterly = max_macro_date_len(6)
    ...
    market_payload = {
        ...
        "macro": {
            "query_date": TARGET,
            "latest_actual_data_date": macro_latest_date,
            "latest_monthly_period_end": macro_latest_monthly,
            "latest_quarterly_period_end": macro_latest_quarterly,
            "effective_period_note": "口径日期由macro indicator原始返回动态提取；FORECAST_END_DATE与尚未到期的MLF_END_DATE不计入最新实际数据日。",
