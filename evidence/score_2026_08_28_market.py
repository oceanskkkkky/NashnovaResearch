...
payload = {
    ...
    "dimensions": [
        {
            ...
            "status": "指数分化回调，上证5日仍正收益，深证与创业板5日走弱，60日趋势均未扭转",
            "leaf_scores": [
                {"name": "上证指数趋势", "score": 60, "basis": f"收盘{trade['CLOSE_PRICE_SZZS']}，日涨跌{trade['CHANGE_PCT_SZZS']}%；5日{interval['CHG_5D_SZZS']:+.2f}%、20日{interval['CHG_20D_SZZS']:+.2f}%、60日{interval['CHG_60D_SZZS']:+.2f}%。"},
                {"name": "深证成指趋势", "score": 52, "basis": f"收盘{trade['CLOSE_PRICE_SZCZ']}，日涨跌{trade['CHANGE_PCT_SZCZ']}%；5日{interval['CHG_5D_SZCZ']:+.2f}%、20日{interval['CHG_20D_SZCZ']:+.2f}%、60日{interval['CHG_60D_SZCZ']:+.2f}%。"},
                {"name": "创业板指趋势", "score": 47, "basis": f"收盘{trade['CLOSE_PRICE_CYBZ']}，日涨跌{trade['CHANGE_PCT_CYBZ']}%；5日{interval['CHG_5D_CYBZ']:+.2f}%、20日{interval['CHG_20D_CYBZ']:+.2f}%、60日{interval['CHG_60D_CYBZ']:+.2f}%。"},
            ],
        ...
        },
        {
            ...
            "status": "指数回调但上涨家数仍占优，涨停活跃，新高数量明显多于新低",
            "leaf_scores": [
                {"name": "上涨占比", "score": 61, "basis": f"上涨{updown['CNT_RED']}、下跌{updown['CNT_GREEN']}、平盘{updown['CNT_ZERO']}，上涨占比{updown['RATIO_UP']}%。"},
                {"name": "涨跌停结构", "score": 74, "basis": f"带日期口径涨停{updown['CNT_REACH_UPLIMIT']}、跌停{updown['CNT_REACH_DNLIMIT']}。"},
                {"name": "新高新低结构", "score": 72, "basis": f"5日新高/新低{updown['CNT_HIGH5']}/{updown['CNT_LOW5']}，20日{updown['CNT_HIGH20']}/{updown['CNT_LOW20']}，60日{updown['CNT_HIGH60']}/{updown['CNT_LOW60']}。"},
            ],
        ...
        },
        {
            ...
            "status": "成交略低于前日，仍高于5日均量，接近10日均量但低于20日及中长期均量",
            "leaf_scores": [
                {"name": "两市成交额", "score": 59, "basis": f"两市成交{trade['MONEY']:.2f}亿元，为5日均量{trade['MONEY_5DAVG_RATIO']:.2f}%、10日均量{trade['MONEY_10DAVG_RATIO']:.2f}%。"},
                ...
                {"name": "量价配合", "score": 55, "basis": "指数回调而个股涨多跌少，成交较前日小幅减少，反映结构性轮动而非全面放量上攻。"},
            ],
        },
        {
            ...
            "degradation": "宏观低频数据不代表8月28日当日变化。",
        },
        {
    ...
    "result": {
        ...
        "market_state": state,
        "summary": "指数回调但个股涨多跌少，成交维持高位且低于20日均量；行业机会转向农业、化工和地产等轮动，维持中性而非全面进攻。",
    },
    "confidence": {
    ...
    "research_verification": {
        ...
        "purpose": "核验2026-08-28盘后市场主线与重要行业催化。",
